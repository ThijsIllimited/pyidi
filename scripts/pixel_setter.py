...
def play_video(video, frame_range, interval=30, points = None, axis = None, show_saturation = False, bit_depth = 16, color = 'r', include_W = False, roi_size = (11,11)):
    ...
    text = ax.text(0.65, 0.9, '', transform=ax.transAxes, color='white', ha='right', va='bottom')
    if axis is not None:
        ax.set_xlim(axis[0])
        ax.set_ylim(axis[1])
    if show_saturation:
    ...
    def update(i):
        im.set_data(video.reader.get_frame(frame_range[i]))
        text.set_text(f'Frame {i}')
        if show_saturation:
            over_sat = np.where(video.reader.get_frame(frame_range[i]) > int(0.99*(2**bit_depth-1)))
            over_sat_plot[0].set_data(over_sat[1], over_sat[0])
            under_sat = np.where(video.reader.get_frame(frame_range[i]) < int(0.01*(2**bit_depth-1)))
