...
class LargeDisplacementOpticalFlow(IDIMethod):
    """
    ...
    def calculate_displacements(self, video):
        ...
        for i, (image_ref, image) in enumerate(p_bar(zip(limited_mraw[:-1],limited_mraw[1:]), ncols=100)):
            # update the gradient parameters
            self.gradient_magnitude, signs_0, signs_1, \
                self.direction_correction_0, self.direction_correction_1 = self.update_gradients(image_ref, video.points)
            ...
            self.latest_displacements = (image_ref[video.points[:,0] + self.delta_0, video.points[:, 1] + self.delta_1] - self.image_roi) / \
    ...
    def update_gradients(self, image, points):
        """Updating the gradients, gradient directions, gradient magnitudes and the reference image.
        In order to save time only the pixels in the neighbourhood of the points are updated.

        :param images: Images to average. The current figure and the next one.
        :param subset_size: Size of the subset to average.
        """
        gradient_0, gradient_1 = np.gradient(image)
        gradient_magnitude = np.sqrt(gradient_0**2 + gradient_1**2)
        signs_0 = np.sign(
            gradient_0[points[:, 0], points[:, 1]])
        signs_1 = np.sign(
            gradient_1[points[:, 0], points[:, 1]])
        direction_correction_0 = np.abs(
            gradient_0[points[:, 0], points[:, 1]] / gradient_magnitude[points[:, 0], points[:, 1]])
        direction_correction_1 = np.abs(
            gradient_1[points[:, 0], points[:, 1]] / self.gradient_magnitude[points[:, 0], points[:, 1]])
        return gradient_magnitude, signs_0, signs_1, direction_correction_0, direction_correction_1
